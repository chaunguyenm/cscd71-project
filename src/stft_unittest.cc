...
namespace {
  TEST(FFTTest, Real) {
    ...
    for (int j = 0; j < num_samples - window_size + 1; j++)
    {
      ...
      for (int i = 0; i < window_size; i++)
      {
        EXPECT_NEAR(out[i][0], stft[j][i].real(), THRESHOLD) << "Incorrect real part at (" << j << "," << i << ").";
        EXPECT_NEAR(out[i][1], stft[j][i].imag(), THRESHOLD) << "Incorrect imaginary part at (" << j << "," << i << ").";
      }
    }
    fftw_destroy_plan(p);
    fftw_free(in); 
